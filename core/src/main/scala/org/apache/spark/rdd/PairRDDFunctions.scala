  ...
  with Serializable
{
      ...
      conf: Configuration = self.context.hadoopConfiguration): FutureAction[Unit] =
  {
    ...
    job.setOutputValueClass(valueClass)
    job.setOutputFormatClass(outputFormatClass)
    SparkHadoopUtil.get.getConfigurationFromJobContext(job).set("mapred.output.dir", path)
    saveAsNewAPIHadoopDatasetAsync(SparkHadoopUtil.get.getConfigurationFromJobContext(job))
  }
